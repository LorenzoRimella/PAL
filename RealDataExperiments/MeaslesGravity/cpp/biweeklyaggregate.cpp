...
// [[Rcpp::export(name=PoiBiweeklyGravitypar)]]
List  PoiBiweeklyGravitypar(arma::vec n,
                           arma::mat y,
                           arma::mat init_dist,
                           arma::mat v,
                           arma::mat births,
  ...
  int T = y.n_rows;
  // Get observations in the right form, each element of observations field is city k's transitions cube
  field<arma::cube> observations(k);

  for(int i=0; i<k; i++){
    //initialise k'th city's observation cube
    arma::cube A(m, m, T, fill::zeros);
    for(int t=0; t<T;t++){
      // fill I->R transitions with observations
      A(2,3,t) = y(t,i);
    }
    // store in k'th city's observation cube
    observations(i) = A;
  }
  
  // initialise filtered intensity matrices for each city
  field<arma::cube>  Lambda_(k);
  field<arma::cube>  Lambda(k);
  field<arma::cube>  Norms(k);
  
  //
  for(int i=0; i<k; i++){
    // times T by 4 to fit aggregation model
    arma::cube A(m, m, T*4+1, fill::zeros);
    arma::cube B(m, m, T, fill::zeros);
    arma::cube C(m, m, T, fill::zeros);
    Lambda_(i) = A;
    Lambda(i) = B;
    Norms(i) = C;
  }
  ...
  for(int t=0; t<T; t++){
    ...
    for(int s = 0; s<4; s++){
      mat a = lambda.slice(t*4+s);
      calculate_beta(betas, v, n, b, a.row(2).t(), k, ones);
...
#pragma omp parallel for num_threads(ncores)
    ...
    for(int i=0; i<k; i++){
      ...
      // Prediction step
      C.slice(t*4+s) = prediction_step(betas(i), K, a.col(i),ones, h);
      Lambda_(i) = C;
      
      lambda.slice(t*4+s+1).col(i) = sum(C.slice(t*4+s),0).t();
...
#pragma omp parallel for num_threads(ncores)
    ...
    for(int i=0; i<k; i++){
      ...
      cube C = Lambda_(i);
      cube E = Norms(i);
      mat Norm = C.slice(t*4)+C.slice(t*4+1)+C.slice(t*4+2)+C.slice(t*4+3);
      E.slice(t) = Norm;
      Norms(i) = E;
      // build Q and obs mat
      mat Q(m,m, fill::zeros);
      Q(2,3) = q(i);
      cube Y = observations(i);
      
      // create matrix for likelihood computation
      mat LQ = Norm%Q;
      // update step
      D.slice(t) = update_step(C.slice(t*4+3), Norm ,Q,Y.slice(t), ones);

      Lambda(i) = D;
      
      // update the intensity vectors.
      // take deaths
      if(t != T-1){lambda.slice(t*4+4).col(i) = (sum(D.slice(t),0).t())%survivalprob ;
      ...
      if(t>tstar){
        w(i,t) = calculate_likelihood(LQ, Y.slice(t));
      }
    }
  }
...
// param is (beta, rho, gamma)
// [[Rcpp::export(name=notparPoiBiweeklyGravitypar)]]
List  notparPoiBiweeklyGravitypar(arma::vec n,
                                  arma::mat y,
                                  arma::mat init_dist,
                                  arma::mat v,
                                  arma::mat births,
...
// param is (beta, rho, gamma)
// [[Rcpp::export(name=PoislowGravity)]]
List  PoislowGravity(arma::vec n,
                            arma::mat y,
                            arma::mat init_dist,
                            arma::mat v,
                            arma::mat births,
  ...
  // initialise filtered intensity matrices for each city
  cube  Lambda_(m,m,4*T, fill::zeros);
  ...
  for(int t=0; t<T; t++){
    ...
    for(int s = 0; s<4; s++){
      calculate_beta(betas, v, n, b, lambda.slice(t*4+s).row(2).t(), k, ones);
...
#pragma omp parallel for num_threads(ncores)
      ...
      for(int i=0; i<k; i++){
        ...
        // Prediction step
        Lambda_.slice(i+s) = prediction_step(betas(i), K, lambda.slice(t*4+s).col(i),ones, h);
        
        lambda.slice(t*4+s+1).col(i) = sum(Lambda_.slice(i+s),0).t();
        lambda(0,i,t*4+s+1)  += ((1-c)/(26*4))*births(t,i);
      }
    }
...
#pragma omp parallel for num_threads(ncores)
    ...
    for(int i=0; i<k; i++){
      ...
      mat Norm = Lambda_.slice(i)+Lambda_.slice(i+1)+Lambda_.slice(i+2)+Lambda_.slice(i+3);
      ...
      mat LQ = Norm%Q;
      // update step
      Lambda.slice(i) = update_step(Lambda_.slice(i+3), Norm ,Q,Y, ones);
  ...
  List out = List::create(Named("lambda") = lambda.slice(4*468));
  return(out);
}
