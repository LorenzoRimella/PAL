...
def school_term_and_school_year(T, intermediate_steps, term, school):
    ...
    return is_school_term_array, is_start_school_year_array
...
def scan_intermediate(X_tprev, K_tprev, is_start_school_year_array_t_obs, intermediate_steps, UKbirths_t, c, n_cities, delta_year):
    ...
    def body(input, t_intermediate):
        ...
        alpha_tm1 = c*UKbirths_t*is_start_school_year_array_t_obs[t_intermediate] + ((1-c)/(26*intermediate_steps))*UKbirths_t*(1-is_start_school_year_array_t_obs[t_intermediate])

        birth_tm1 = sample_poisson(alpha_tm1)
        ...
        X_tm1_bar = X_tm1 + X_birth_tm1 - death_tm1

        Z_t = sample_multinomial(X_tm1_bar, K_tprev)
        X_t = tf.reduce_sum(Z_t, axis = 1)
...
@tf.function(jit_compile=True)
def body_run(X_t, intermediate_steps, UKbirths_t, pop_t, beta_bar, p, a, is_school_term_array_t, is_start_school_year_array_t_obs, h, rho, gamma, xi_t, q_t, c, n_cities, delta_year, v):

    infection_rate = compute_infection_rate(X_t, is_school_term_array_t, pop_t, p, a, beta_bar, v, xi_t)
    K_t = assemble_K(h, infection_rate, rho, gamma)

    X_t, Z_t = scan_intermediate(X_t, K_t, is_start_school_year_array_t_obs, intermediate_steps, UKbirths_t, c, n_cities, delta_year)
...
def run(T, intermediate_steps, UKbirths, UKpop, g, measles_distance_matrix, initial_pop, pi_0, beta_bar, p, a, is_school_term_array, is_start_school_year_array, h, rho, gamma, Xi, Q, c, n_cities, delta_year):
    ...
    X_t, Y_t = tf.scan(body, tf.range(0, T+1, dtype=tf.int64), initializer = (X_0, Y_0)) 
