...
simulation_name = "seq_optimization_B_05_2_trance"
...
# optimal_shared_param = np.load("PAL_cineca/Data/Output/optimization_A_01.npy")
...
x_to_start = np.load("PAL_cineca/Data/Output/seq_optimization_B_05.npy")
...
seq_x_to_start = x_to_start

for i in range(16, 40):
    
    def seq_optimization_func(seq_x_0_to_optim):
        
        seq_x_to_start[3*i:3*(i+1)] = seq_x_0_to_optim
    
        return optimization_func(seq_x_to_start)
    
    seq_x_0_to_start = seq_x_to_start[3*i:3*(i+1)]
    seq_bnds = bnds[3*i:3*(i+1)]

    start = time.time()
    res = minimize(seq_optimization_func, seq_x_0_to_start, bounds = seq_bnds, method='SLSQP', options ={"eps":0.5, "maxiter":500})
    print("Running time for the "+str(i)+" city is ", str(time.time()-start))

    string1 = ["################################", "\n"]
    string2 = ["The max for the "+str(i)+" city is ", "\n"]
    string3 = [" "+str(res.x), "\n"] 
    f= open("PAL_cineca/Data/Check/"+simulation_name+".txt", "a")
    f.writelines(string1)
    f.writelines(string2)
    ...
    seq_x_to_start[3*i:3*(i+1)] = res.x

    np.save("PAL_cineca/Data/Output/"+simulation_name+".npy", seq_x_to_start)

def seq_optimization_func(seq_x_rest_to_optim):

    seq_x_to_start[3*39:] = seq_x_rest_to_optim

    return optimization_func(seq_x_to_start)

seq_x_rest_to_start = seq_x_to_start[3*39:]
seq_bnds = bnds[3*39:]

start = time.time()
res = minimize(seq_optimization_func, seq_x_rest_to_start, bounds = seq_bnds, method='SLSQP', options ={"eps":0.5, "maxiter":500})
print("Running time for the rest is ", str(time.time()-start))

string1 = ["################################", "\n"]
string2 = ["The max for the rest is ", "\n"]
string3 = [" "+str(res.x), "\n"] 
f= open("PAL_cineca/Data/Check/"+simulation_name+".txt", "a")
f.writelines(string1)
f.writelines(string2)
